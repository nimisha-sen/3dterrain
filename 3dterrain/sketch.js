...
- Water/sea height changes w.r.t to sealevel pressure in a specific location
- the other elements are randomly changing, although can be linked to the sealevel in future
- due to the sealevel pressure values being too minuscule for the scope of the project-the changes are difficult to see, 
but the values are changing in the console
*/
...
function setup(){
  createCanvas(700, 600, WEBGL); //WEBGL=is a JS api for rendering interactive 2D & 3D graphics without the use of plugins
    
  // Fetch JSON
    fetchWeatherDataInInterval(10000);
  
  //Create slider
  timeSlider = createSlider(0, 23, 0);
...
function draw() {
  ...
  if(isWeatherloaded === false) {
    text(`Invalid JSON!`,20,20);
      return;
    }

  // Sealevel pressure related 
  let pressureUnit = weatherjson.hourly_units.pressure_msl;
  seaLevel = weatherjson.hourly.pressure_msl[timeSlider.value()];
  ...
  rotateY(-t * rotateScale); //the CSS rotateY()func is used to rotate elements around the y-axis in a 3D space 
  
  // Some lighting
  ambientLight(100, 100, 100); //without this it looks minecraft-esque 
  directionalLight(200, 200, 200, -1, 0.75, -1);
  
  drawTerrain(t);
}

function keyReleased() {  //not necessary to include
...
function drawTerrain(t) {
  ...
  if(heightScale != 0) {
    push();
    const waterHeight = getBoxHeight(seaLevel / 2000);
    ...
    box(waterSize, getBoxHeight(seaLevel / 2000), waterSize);
    pop();
  }
}
